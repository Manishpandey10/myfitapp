...
const cleanPlanRaw = (raw: string) => {
  let text = raw.trim();

  // If it starts with ``` then strip the fences
  if (text.startsWith('```')) {
    // Remove starting ```json or ``` plus any whitespace/newlines
...
export default function HomePage() {
  ...
  const handleSubmit = async (formData: any) => {
    ...
    try {
      ...
      if (result?.planRaw) {
        let plan: any = result.planRaw;

        // If it's a string with ```json ... ``` code, clean & parse it
        if (typeof plan === 'string') {
          try {
            const jsonString = cleanPlanRaw(plan);
            plan = JSON.parse(jsonString);
          } catch (e) {
            console.error('Failed to parse planRaw JSON:', e);
            setError('Got invalid JSON from AI. Please try again.');
            setPlan('');
            setReadyToView(false);
            return;
          }
        ...
        // Store the parsed object in Zustand
        setPlan(plan);
        setReadyToView(true);
        setError(null);
      } else {
